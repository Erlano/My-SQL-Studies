...
SELECT CONCAT(first_name, ' ', last_name, '. ', gift, ' – cena: ', price) FROM aggregates.batman;
...
SELECT last_name, CHAR_LENGTH(last_name) AS 'Długość' FROM aggregates.batman;

-- Wyświetl pierwsze dwie litery imion.
SELECT LEFT(first_name, 2) FROM aggregates.batman;

-- Wyświetl imię, nazwisko i login użytkownika w formacie: mała litera imienia + 3 pierwsze, małe litery nazwiska.
SELECT
	first_name AS 'Imię',
    last_name AS 'Nazwisko',
    CONCAT(LOWER(LEFT(first_name, 1)), LOWER(LEFT(last_name, 3))) AS 'Login'
